...
void getEnvPackage (link_datatype *buf)
{
	...
	tmp = pack.lux;
	current.illumination = (unsigned short)tmp;//光强

	tmp = (pack.gas & 0xffff0000) >> 16;
	current.gas = (unsigned short)tmp;//烟雾
	
	printf ("%04x %04x \n",current.illumination,current.gas);

	printf ("ep_no = %d tem = %0.2f hum = %0.2f ill = %d battery = %d adc = %0.2f x = %d y = %d z = %d\n", ep_no,
			current.temperature, current.humidity, current.illumination, current.gas, current.adc, current.x, current.y, current.z);
