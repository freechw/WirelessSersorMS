...
void osal_run_system( void )
{
  ...
  do {
    if (tasksEvents[idx])  // Task is highest priority that is ready.// T务是准备好的最高优先级
    {
  ...
  if (idx < tasksCnt)
  {
    ...
    tasksEvents[idx] |= events;  // Add back unprocessed events to the current task.// 保存未处理的事件 将未处理的事件返回到当前任务。
    HAL_EXIT_CRITICAL_SECTION(intState);// 退出临界区
  }
#if defined( POWER_SAVING )
  else  // Complete pass through all task events with no activity?
  {
    osal_pwrmgr_powerconserve();  // Put the processor/system into sleep// 使处理器/系统进入休眠状态
  }
#endif
