...
static void rxCB(uint8 port,uint8 event)
{
	...
	HalUARTRead(0,&cmd,1);//�Ӵ��ڶ�ȡ1���ֽڵ����ݵ�cmd��
...
void GenericApp_Init( uint8 task_id )
{
  ...
  GenericApp_TransID = 0;//消息发�?ID（多消息时有顺序之分�?  
...
uint16 GenericApp_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
  //接收属于本应用任�?GenericApp的消息，�?GenericApp_TaskID 标记
    ...
    while ( MSGpkt )
    {
      ...
      // Release the memory// 释放内存 //事件处理完了，释放消息占用的内存
